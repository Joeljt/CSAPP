...
void eval(char *cmdline)
{
    char *argv[MAXARGS]; /* 存放 parseline 解析后的命令行参数 */
    char buf[MAXLINE];   /* 存放 parseline 解析后的 */
    int bg;              /* 当前命令是否是后台任务 */
    pid_t pid;           /* 进程 ID */
    sigset_t mask;       /* 屏蔽信号，避免并发问题 */

    // 将用户输入的内容拷贝下来，在 parseline 中做一些格式化操作
    strcpy(buf, cmdline);

    // 解析命令行内容，根据命令行是否是 & 结尾决定是不是后台进程
    bg = parseline(buf, argv);

    // 如果没有输入任何内容，直接 return，相当于在命令行中输入了空行
    if (argv[0] == NULL)
        return;
    ...
    if (!builtin_cmd(argv))
    {
        ...
        if ((pid = fork()) == 0)
        {
            ...
            // 将子进程放到一个新的进程组里，确保前台执行的时候只有子进程可以捕获到 SIGINT 信号
            setpgid(0, 0);
        ...
        // 父进程
        // 将刚创建的子进程添加到作业列表
        // 传入的是 cmdline 而不是 buf，因为可能需要打印出来，显示原始的命令
        addjob(jobs, pid, bg ? BG : FG, cmdline);

        // 在成功 fork 子进程以后解除 SIG_CHLD 信号的屏蔽
 ...
 *  - s1 > s2 返回 1，s1 < s2 返回 -1，s1 == s2 返回 0
 */
int builtin_cmd(char **argv)
{
